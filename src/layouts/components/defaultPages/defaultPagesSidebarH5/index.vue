...
<template>
  <div class="SidebarH5-top-view">
    ...
    <div style="margin-left:auto;" @click="handleClose">
      <svg v-if="activeThemeName=='normal'" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="navlist">
    <div class="nav-item"  :class="{ 'nav-item-active': currentTab === index }" @click="changeTab(item,index)" v-for="(item,index) in headerList" :key="iindex">{{item.title}}</div>
  </div>
  <div class="SidebarH5-bottom-view" >
    <div class="login-view" v-if="userStore.token">
      <div class="header">5</div>
      <div class="right">
        <p>Fwdadadws</p>
        <p>Fawdasda@gmail.com</p>
      </div>
    </div>
    <div v-else class="notlogin-view">登录</div>
  </div>
